		...
		<div id="list-todoItem" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="name" title="${message(code: 'todoItem.name.label', default: 'Name')}" />
					
						<g:sortableColumn property="isDone" title="${message(code: 'todoItem.isDone.label', default: 'Is Done')}" />
				...
				<g:each in="${todoItemInstanceList}" status="i" var="todoItemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${todoItemInstance.id}">${fieldValue(bean: todoItemInstance, field: "name")}</g:link></td>
					
						<td><g:formatBoolean boolean="${todoItemInstance.isDone}" /></td>
